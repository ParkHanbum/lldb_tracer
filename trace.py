...
def trace(debugger, command, result, internal_dict):
    ...
    while True:
        ...
        if  (	instrumented_frame == None or 
                    (   not instrumented_frame.is_stopped_on_call(frame) and 
                        not instrumented_frame.is_stopped_on_syscall(frame) and 
                        not instrumented_frame.is_stopped_on_jmp(frame, True)
                    )
            ):
